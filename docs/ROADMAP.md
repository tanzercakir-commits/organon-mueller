# organon-mueller — Yol Haritası (TASLAK, frozen-N Aşama 2 sonunda ilan edilir)

Tahmin: **~22 aşama, 6 faz**. Organon v1 geleneği: taslak erken çizilir,
N sayısı çekirdek mimari oturunca DONDURULUR ve sonra değişmez (v1'de 55).
Kuntman/grup geri bildirimi C ve D fazlarının sırasını değiştirebilir.

```
FAZ A — Zemin (Aşama 0–2)
├── 0  Temsil katmanı + 14 özdeşlik regresyonu            ✅ (36/36)
├── 1  Kütüphane genişletme (21 özdeşlik) + serileştirme
│      + egglog spike (BAŞARILI, hibrit mimari önerisi)    ✅ (50/50)
└── 2  Keşif motoru çekirdeği v0: hibrit egglog+SymPy,
       kanonik form, ilk enumerasyon → FROZEN-N İLANI

FAZ B — Keşif motoru (Aşama 3–7)
├── 3  Terim enumerasyonu + karmaşıklık sınırları
├── 4  Aday boru hattı: sayısal ön-elek → sembolik ispat
├── 5  Geri-kazanım kampanyası: motor I1–I21'i KENDİSİ bulmalı
├── 6  Yeni aday taraması #1 + literatür karşılaştırma disiplini
└── 7  İterasyon değerlendirmesi (v1 iter geleneği)

FAZ C — Ayrışım türetici (Aşama 8–11)
├── 8  Simetri-koşullu ayrışım: AO2016'nın 6 tipinin otomatik türetimi
├── 9  Rank-2 genel çözücü
├── 10 Rank-3 keşif taraması (yayın-aday yeni sonuç potansiyeli)
└── 11 İter + Kuntman feedback penceresi #1

FAZ D — Dipol modülü (Aşama 12–15)
├── 12 Coupled-dipole sembolik motor (PRB 98,045410 yeniden türetim)
├── 13 γ (optik aktivite) parametresinin yön-genel otomasyonu
├── 14 N-dimer / ensemble genellemesi (OA-ensemble açık uçları)
└── 15 İter + feedback penceresi #2

FAZ E — Paketleme (Aşama 16–19)
├── 16 LaTeX/rapor üretici (bulgu → makale malzemesi)
├── 17 MCP server (verify_identity, find_identities, derive_decomposition...)
├── 18 Web arayüzü (Streamlit/Gradio; hosted)  [opsiyonel]
└── 19 Dokümantasyon + örnek defterler

FAZ F — Kapanış (Aşama 20–22)
├── 20 Yayın-aday sonuçların konsolidasyonu
├── 21 Dış doğrulama (grup feedback, bağımsız tekrar)
└── 22 v2.0 kapanış değerlendirmesi + retrospektif
```

Süre kalibrasyonu (handoff): sıkı odakta 2–4 ay, part-time 6–12 ay.
Aşama ≈ v1'deki milestone tanesi; her aşama spec → impl → test → rapor
döngüsüyle kapanır.
